...
void printMap(vector<map<int, int>>& data, int m, int n) {
    int i = 0;
    int j = 0;
    ...
    for (int ind = 0; ind < data.size(); ++ind) {
        i = (ind / 2) / m;
        j = (ind / 2) % m;
        cout << "\n coord: " << i << ' ' << j << " type: " << (ind % 2) <<" :\n";
        for (auto elem : data[ind]) {
            v = elem.first;
            c = elem.second;
            cout << "    coord: " << (v / 2) / m << ' ' << (v / 2) % m << " type: " << (v % 2) << " cap: " << ' ' << c << '\n';
        }
    }
}
...
void getMaxFlow() {
    ...
    printMap(residualNetwork, field.size(), field[0].size());
